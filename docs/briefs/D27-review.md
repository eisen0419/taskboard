...
4. **面板其它分支零 diff**：`git diff $base..HEAD | grep -E '^[-+][^-+]' | grep -cE 'setSelectedProjectId|openTaskDetail|setPaletteOpen|setPaletteQuery'` = **0**（只数改动行；hunk 上下文里的 `setSelectedProjectId(ALL_PROJECTS_ID);` 不算，整 diff 直接 grep -c 得 1 是预期，ESCALATION-10 勘正）。
