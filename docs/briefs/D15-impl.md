...
- `test/**` 与 `InboxView.test.tsx` 既有用例不删不改，唯二例外：① `inbox SSE broadcasts created and updated events` 的 5 处字面值（见扩展点，`git diff $base..HEAD -- test/inbox.test.mjs | grep -E '^-' | grep -vE '^---' | wc -l` ≤ 5 且全在该用例块内）② 既有 4 条组件 fixture 补 `collapsedCount: 1` 字段。
