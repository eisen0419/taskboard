...
1. 断言被删或放宽：**保留**测试文件的删除行里含 `assert|expect`，**只在**该删除行出现在实现 report 的「文件:行 → 删了什么 → 对应哪个被删面」表里、且对应面确在验收①清单内时不算（ESCALATION-67 D30：允许删以被删面为对象的用例 / 断言 / import）；表外的删除行、把期望值改成能过、新增 `@ts-ignore|@ts-nocheck|\.skip\(|\.only\(` = 命中。整文件删除：清单内不算；清单外仅当 report 写明「N/N 用例对象均为被删面」且你抽核成立时不算。
