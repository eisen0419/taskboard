...
> 🔴 **先读议题 <https://github.com/eisen0419/taskboard/issues/5>（`gh issue view 5 --comments`）——Spec 轴的判据真源是它的「验收」8 条与「Out of scope」5 条。**

席位：`grok`（实现者是 `agy-flash`，跨家；**docs-only 轻审轨**，ADR-0013 第 4 条 / ADR-0014 D64：Spec 轴 + 归店 + 脱敏，不跑 Standards 轴）。
实现 report：`/Users/happy/projects/taskboard/.scratch/d5-native/reports/impl.md`，从它取实现 commit sha。
...
**Spec 轴**：逐条对议题「验收」8 条，判「做到 / 半做 / 没做 / 做错」，每条贴你自己跑的命令与取值。①–⑧ 的判据命令议题里已写死，**逐字跑**，④⑤ 的 `while` 循环也要跑（不是看实现 report 抄数）；⑧ 末句的「协调席亲跑 rev-parse / porcelain」你也跑一次。先把条数写在 verdict 开头。Out of scope 5 条做了也算做错，尤其：动了 multica-upstream（`git -C /Users/happy/projects/multica-upstream status --porcelain | wc -l` ≠ 0 或 HEAD 变了）、改了 taskboard 其他文件、写了实施方案细节或 ADR、与其他产品比较。

**归店**：交付物只在 `docs/research/D5/multica-reference.md`，`git diff --stat $(git merge-base origin/main HEAD)..HEAD` 恰 1 文件；「我已经跑过的」协调席取值没有被写成实现席自己跑的（任务书 D5-research「我已经跑过的」块里的数字若原样出现在报告，核报告 `## 取值` 里有没有对应的自跑命令）。
...
必修：验收 ①–⑧ 任一条不达；条款④ 命中（编路径 / 编取值，含抽核 1 的「路径不存在」）；「我没能确认的」写「无」或空；动了 multica-upstream 或 taskboard 其他文件；commit 数 ≠ 1。
...
4. `docs/briefs/**` `docs/agents/**` `docs/research/**`（除 `docs/research/D5/multica-reference.md`）`AGENTS.md` `CLAUDE.md` `.teams-orca*.json` 改动 = 命中，本议题无豁免。
...
VERDICT 恰一个 commit：`git commit -m "docs(research): D5 R1-VERDICT (#5)" -- docs/research/D5/R1-VERDICT.md`；禁 `git add -A`；去 trailer 配方同实现单；🔴 禁 `git push`。

## 硬规则

1. 🔴 **只审不改**：不改交付物、不改任何别的文件。
2. 🔴 禁 `git push`。
3. 🔴 产物 = `docs/research/D5/R1-VERDICT.md`，头行 `PASS` / `FAIL`，按「Spec 轴 → 归店与脱敏 → 证据强度抽核 → 局限」四节写。
...
report 落 `/Users/happy/projects/taskboard/.scratch/d5-native/reports/review.md`（`mkdir -p`），同目录 `touch .DONE`，然后**恰一次** `orca orchestration send --type worker_done --subject "PASS" --body "<三句>" --task-id <taskId> --dispatch-id <dispatchId> --outcome succeeded --files-modified "docs/research/D5/R1-VERDICT.md" --report-path /Users/happy/projects/taskboard/.scratch/d5-native/reports/review.md --json`。**`worker_done` 是最后一步，发完立即停手。**
